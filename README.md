...
- `src/router.tsx` creates a fresh `QueryClient` in `getRouter()` and wires it to
  `setupRouterSsrQueryIntegration(...)`
- `src/routes/__root.tsx` types the router context and relies on that integration to provide
  `QueryClientProvider`
...
to the root route will appear in all the routes.
